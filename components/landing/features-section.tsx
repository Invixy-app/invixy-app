...
export const features = [
  {
    ...
    color: "bg-[var(--brand-cobalt)]/10",
    iconColor: "text-[var(--brand-cobalt)]",
    visual: "speed"
  },
  {
    id: "feature-2",
...
export function FeaturesSection() {
  ...
  return (
    <section id="features" className="relative bg-background">
      <div
        ref={containerRef}
        className="container mx-auto flex flex-col px-6 lg:flex-row lg:px-8"
      >
        
        {/* Left Column - Scrolling Cards */}
        <div className="w-full space-y-24 py-12 lg:w-1/2 lg:py-24 lg:space-y-[30vh]">
          {features.map((feature, index) => (
            <div
              key={feature.id}
              ref={(el) => { cardsRef.current[index] = el }}
              className="flex flex-col justify-center py-2 pr-2 md:py-4 md:pr-8"
            >
        ...
        <div 
          ref={rightColRef} 
          className="sticky top-0 hidden h-screen w-1/2 items-center justify-center overflow-hidden p-10 xl:p-12 lg:flex"
        >
              ...
              >
                ...
                {feature.visual === 'speed' && (
                    <div className="flex flex-col items-center gap-6 p-8 w-full max-w-xs">
                        <div className="relative w-48 h-24 overflow-hidden">
                            ...
                            <div className="absolute top-0 left-0 w-full h-48 rounded-full border-[12px] border-[var(--brand-cobalt)] border-b-transparent border-r-transparent border-l-transparent transform -rotate-[45deg]" />
                        </div>
                        ...
                        <div className="flex gap-2 text-xs bg-[var(--brand-teal)]/10 text-[var(--brand-teal)] px-3 py-1 rounded-full items-center">
                ...
                {feature.visual === 'security' && (
                    <div className="w-full h-full p-8 flex flex-col items-center justify-center bg-grid-white/[0.02] bg-[size:16px_16px]">
                        <div className="w-24 h-24 bg-[var(--brand-teal)]/20 rounded-full flex items-center justify-center mb-6 animate-pulse">
                          <Lock className="w-12 h-12 text-[var(--brand-teal)]" />
                        </div>
                    ...
                    <div className="w-full max-w-sm p-8 flex flex-col gap-4">
                        <div className="bg-background border rounded-lg p-4 shadow-sm">
                            <div className="flex justify-between items-end h-24 gap-2">
                                <div className="w-full bg-[var(--brand-cyan)]/20 h-[40%] rounded-sm" />
                                <div className="w-full bg-[var(--brand-cyan)]/30 h-[60%] rounded-sm" />
                                <div className="w-full bg-[var(--brand-cyan)]/40 h-[50%] rounded-sm" />
                                <div className="w-full bg-[var(--brand-cyan)]/50 h-[80%] rounded-sm" />
                                <div className="w-full bg-[var(--brand-cyan)] h-[70%] rounded-sm" />
                            </div>
                        </div>
                        <div className="flex items-center gap-4">
                             <div className="flex-1 bg-background border rounded-lg p-3">
                                ...
                                <div className="text-lg font-bold flex items-center gap-2">
                                    $24k <TrendingUp className="w-4 h-4 text-[var(--brand-teal)]" />
                                </div>
                             </div>
                             <div className="flex-1 bg-background border rounded-lg p-3">
                                <div className="text-xs text-muted-foreground">Growth</div>
                                  <div className="text-lg font-bold text-[var(--brand-teal)]">+12%</div>
                             </div>
                        </div>
                    </div>
                )}
